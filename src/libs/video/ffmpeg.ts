...
export class FFmpegProcessor {
    public async mergeVideos(options: {
        ...
        return new Promise((resolve, reject) => {
            ...
            const defaultResolution = options.resolution ?? { width: 1920, height: 1080 };
            let filterComplex = '';
    ...
    public async mergeVideosOld(inputVideos: string[], outputVideo: string): Promise<void> {
        // Create temp file in same directory as output video
        const tempFilePath = path.join(path.dirname(outputVideo), `concat-input-${Date.now()}.txt`);

        try {
            // Build the content for the temporary file
            const fileContent = inputVideos.map((video) => `file '${video}'`).join('\n');
    ...
    public async embedSubtitleTrack(options: ISubtitleEmbedOptions): Promise<void> {
        const { inputVideoFilename, outputVideoFilename } = options;

        const tempSubtitlePath = path.join(options.tmpPath, `subtitles-${options.jobId}-${options.segmentIndex}.ass`);
        const tempAudioPath = options.audioContent
            ? path.join(options.tmpPath, `audio-${options.jobId}-${options.segmentIndex}.mp3`)
            : null;
    ...
    public async embedSubtitleTrackOld(options: ISubtitleEmbedOptions): Promise<void> {
        const tempSubtitlePath = path.join(options.tmpPath, `subtitles-${options.jobId}-${options.segmentIndex}.ass`);
        const tempAudioPath = options.audioContent
            ? path.join(options.tmpPath, `audio-${options.jobId}-${options.segmentIndex}.mp3`)
            : null;
        ...
        try {
            ...
            const audioDuration = tempAudioPath ? await this._getDuration(tempAudioPath) : 0;

            await new Promise<void>((resolve, reject) => {
                const command = ffmpeg().input(options.inputVideoFilename);

                if (tempAudioPath && audioDuration > 0 && audioDuration < videoDuration) {
                ...
                if (tempAudioPath) {
                    command
                        .input(tempAudioPath)
                        .outputOptions(['-map 0:v', '-map 1:a', '-af', 'apad=pad_dur=2', '-shortest']);
                }
                ...
                command
                    .outputOptions([
                        ...
                        ...(options.subtitleLanguage
                            ? ['-metadata:s:s:0', `language=${options.subtitleLanguage ?? 'en'}`]
                            : []),
                    ])
